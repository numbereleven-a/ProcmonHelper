...
- Procmon-owned arguments use `ArgumentList`, preventing option/path concatenation.
- The normal UI runs as the current user. Only the restricted worker is elevated.
- Named pipes are unique per session and limited to the current Windows user.
...
PML, CSV, and XML can contain personal or confidential data, including paths, user names, registry content, and command lines. Users must review the displayed final destination and apply their organization's trace-retention rules.
